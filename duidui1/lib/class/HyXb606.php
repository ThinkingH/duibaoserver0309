...
class HyXb606 extends HyXb{
	...
	public function controller_shouyetypelist(){
		...
		/* $dataarr = parent::func_advertisement();
		print_r($dataarr); */
		...
		$sql_where = '';
		if($this->quantype==''){
			$sql_where .= "z='".$z."' and flag=1 and zflag=1 ";
		}else{
			$sql_where .= "z='".$z."' and flag=1 and zflag=1 and maintype='".$this->quantype."'";
		}
		...
		if($this->page=='' || $this->page=='0' || $this->page=='undefined' ){
			...
			//分类数据的查询
			$typesql  = "select count(*) as num from z_quanmainlist where $sql_where ";
