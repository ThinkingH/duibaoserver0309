...
class HyXb802 extends HyXb{
	...
	public function controller_findproductlist(){
		...
		if($this->quantype=='all'){
			...
			$maintypesql  = "select maintype,count(*) as num from z_tuanmainlist group by maintype ";
			$maintypelist = parent::__get('HyDb')->get_all($maintypesql);
			
		}else{
			$maintypesql  = "select childtype,count(*) as num from z_tuanmainlist  where maintype='".$this->quantype."' group by childtype ";
			$maintypelist = parent::__get('HyDb')->get_all($maintypesql);
		}
