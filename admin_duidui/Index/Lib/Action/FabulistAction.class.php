...
class FabulistAction extends Action {
	...
	public function index(){
		...
		foreach ($list as $keys=>$vals){
			...
			$arr = unserialize(BUCKETSTR);//七牛云存储连接$arr['duibao-find']
			
			//图片展示
			if(substr($list[$keys]['picurl'],0,7)!='http://'){
				$list[$keys]['picurl']=$arr['duibao-find'].$list[$keys]['picurl'].'?imageView2/1/w/100/h/100/q/75|imageslim';
			}
	...
	public function updateshow(){
		...
		if($update_submit!=''){
			...
			//	echo substr($listdata[0]['picurl'],20);
				...
				//图片展示
				if(substr($listdata[0]['picurl'],0,7)!='http://'){
					$listdata[0]['picurl']=$arr['duibao-find'].$listdata[0]['picurl'].'?imageView2/1/w/100/h/100/q/75|imageslim';
				}
			}
	...
	public function updatedata(){
		...
		if($infof===true){
			...
			if(substr($filepath,0,7)=='http://' ||substr($filepath,0,1)=='/' ){
				...
				//云存储上删除
				delqiuniu('duibao-find',$filepath);
			}
