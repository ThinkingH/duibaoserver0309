...
$payarray['over_datetime'] = $over_datetime;
...
if(false!==$ret) {
	...
	//md5(siteid+typeid+stat+paymoney+myorderid+siteorderid+over_datetime+ckey)
	...
	if($mymd5key==$md5key) {
		...
		if($ret) {
			...
			if('DELIVRD'==$payarray['stat']) {
				...
				//商城订单更新状态
				$shoporder_sql = "update shop_userbuy set status='3', zhifu_createtime='".date('Y-m-d H:i:s')."' where status='20' and zhifu_order='".$myorderid."'";
				
				$tmp_logstr  = $shoporder_sql."\n";
				$HyRep->hy_log_str_add($tmp_logstr);
				unset($tmp_logstr);
				
				$HyRep->__get('HyDb')->execute($shoporder_sql);
