...
class HyPay {
	...
	public function __construct($input_data){
		...
		$this->productid        = isset($input_data['productid'])?$input_data['productid']:'';//订单id
		$this->pronum        = isset($input_data['pronum'])?$input_data['pronum']:'';//订单数量
	...
	//用户订单表的插入
	public function func_shop_order_insert(){
		...
		//查找用户的地址
		$sql_address = "select id from xb_user_address where userid='".$this->remark."' and is_default ='1' ";
		$list_address = $this->HyDb->get_row($sql_address);
		
		$product_sql  = "select * from shop_product where id='".$this->productid."' ";
		$product_list = $this->HyDb->get_row($product_sql);
		
		//总金额
		$totalmoney = $product_list['price']*$this->pronum;
		
		$sql_shopinsert = "insert into shop_userbuy (userid,typeid,childtypeid,status,
							mtype,name,productnum,orderno,price,productid,
							pingjia,order_createtime,address_id,) 
						values ('".$this->remark."','".$product_list['typeid']."','".$product_list['typeidchild']."','20',
								'".$product_list['xushi_type']."','".$product_list['name']."','".$this->pronum."','".$this->d_myorderid."',
								'".$totalmoney."','".$this->productid."','9','".date('Y-m-d H:i:s')."','".$list_address['id']."' )";
		
		$this->log_str .= HyItems::hy_trn2space($sql_shopinsert)."\n";
		
		$list_shopinsert = $this->HyDb->execute($sql_shopinsert);
		
		return $list_shopinsert;
