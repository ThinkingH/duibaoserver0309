...
class ShopaddAction extends Action{
			...
			if(count($repeatdatalist)>0){//该数据存在，不可以重复添加
				...
				for ($i=0;$i<=3;$i++){
					...
					if($info[$i]['key']=='mainpic'){
						...
						$data['mainpic'] = URL_APK.str_replace('./','/',$mainpic);
					}
		...
		if($update_submit==''){
			...
			if($id==''){
				...
				foreach($tuijian_arr as $keyc => $valc) {
					...
					if($shoplist[0]['flag']==$keyc) { $optionhottypeid .= ' selected="selected" '; }
	...
	public function deletedata(){
		...
		$datasql = "select mainpic,showpic1 from shop_product where id='".$id."'";
		$datalist = $Model->query($datasql);
		
		if(file_exists($datalist[0]['mainpic'])){
			unlink($datalist[0]['mainpic']);
		}
		
		if(file_exists($datalist[0]['showpic1'])){
			unlink($datalist[0]['showpic1']);
		}
