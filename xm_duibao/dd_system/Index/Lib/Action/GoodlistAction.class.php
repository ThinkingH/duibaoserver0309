 ...
 * 商品的添加--新的分类
 */
class GoodlistAction extends Action{
	...
	public function addshow(){
		...
		$optionreal = '<option value=""></option>';
		foreach($real_arr as $keyc => $valc) {
			$optionreal .= '<option value="'.$keyc.'" ';
			/* if($status==$keyc) { $optionstatus .= ' selected="selected" '; } */
			$optionreal .= '>'.$valc.'</option>';
		}
		$this->assign('optionreal',$optionreal);
		...
		$optionmiyao = '<option value=""></option>';
		foreach($liayuan_miyao as $keyc => $valc) {
			$optionmiyao .= '<option value="'.$keyc.'" ';
			/* if($status==$keyc) { $optionstatus .= ' selected="selected" '; } */
			$optionmiyao .= '>'.$valc.'</option>';
		}
		$this->assign('optionmiyao',$optionmiyao);
		...
		$optioncode = '<option value=""></option>';
		foreach($code_arr as $keyc => $valc) {
			$optioncode .= '<option value="'.$keyc.'" ';
			/* if($status==$keyc) { $optionstatus .= ' selected="selected" '; } */
			$optioncode .= '>'.$valc.'</option>';
		}
		$this->assign('optioncode',$optioncode);
		
		
		//是否包邮
		$free_shippingarr = array(
				'0' => '包邮',
				'1' => '自提',
		);
		$optionshipping = '<option value=""></option>';
		foreach($free_shippingarr as $keyc => $valc) {
			$optionshipping .= '<option value="'.$keyc.'" ';
			/* if($status==$keyc) { $optionstatus .= ' selected="selected" '; } */
			$optionshipping .= '>'.$valc.'</option>';
		}
		$this->assign('optionshipping',$optionshipping);
