...
class KaipingtuAction extends Action {
	...
	public function updatelunbotushow(){
		...
		if(count($verdata_list)<=0){
			...
			$verdata_list[0]['img'] = hy_qiniuimgurl('duibao-basic',$verdata_list[0]['img'],'100','150');
	...
	public function updatelunbotudata(){
		...
		if($infof===true){
			...
			$deldata = delqiuniu('duibao-basic',$seldata_list[0]['img']);
	...
	public function deletelunbodata(){
		...
		if($delete_submit!=''){
			...
			$ret_zhuangtai = M("xb_lunbotu")->where("id='".$id."'")->find();
