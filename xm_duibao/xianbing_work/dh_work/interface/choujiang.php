		...
		if(count($sharelist)>0){
			...
			if(count($selectdatalist)>0){//已进行过分享，可以进行一次抽奖
			 ...
			 if($userjifen<$main_xiaohaoscore){//用户积分小于抽奖消耗的积分
				 ...
				 if($prizelist['num']>=$main_day_max && $main_day_max=='1'){
				 	
					 echo '0';
					 $HySession->set('f_angle','抽奖次数达到上限，分享后还可以进行再次抽奖哦！');
			 	
			 	}else if($prizelist['num']>=$main_day_max && $main_day_max=='2'){
			 		echo '0';
			 		$HySession->set('f_angle','抽奖次数达到上限，请明日再来！');
			 	}else{
				...
				if(count($bioashilist)>0){
					...
					$updatesql = "update choujiang set biaoshi='1',phone='".$phone."',openid='".$openid."',
							...
							where userid='".$userkey."' and biaoshi='9' and choujiangtime>='".$starttime."' and choujiangtime<='".$endtime."' ";
