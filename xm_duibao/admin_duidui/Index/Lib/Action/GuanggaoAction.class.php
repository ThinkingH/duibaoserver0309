...
class GuanggaoAction extends Action {
	...
	public function index(){
		...
		if($list[$keys]['flag']=='1'){
			...
			//图片展示
			if(substr($list[$keys]['img'],0,7)!='http://'){
				$list[$keys]['img']=$arr['duibao-basic'].$list[$keys]['img'].'?imageView2/1/w/200/h/100/q/75|imageslim';
			}
		...
		foreach ($blist as $keys=>$vals){
			...
			//图片展示
			if(substr($blist[$keys]['picurl'],0,7)!='http://'){
				$blist[$keys]['picurl']=$arr['duibao-basic'].$blist[$keys]['picurl'].'?imageView2/1/w/180/h/180/q/75|imageslim';
			}
		
		}
					
		$this -> assign('list',$list);//轮播图广告
		$this -> assign('blist',$blist);//附近广告
	...
	public function updatelunbotudata(){
		...
		if($infof===true){
			...
			if(substr($filepath,0,7)=='http://' ||substr($filepath,0,1)=='/' ){
