...
$HYONEPATH = dirname(__FILE__);
define('HYONEPATH', $HYONEPATH); //定义核心操作根目录

//引入数据库链接参数文件
require( $HYONEPATH.'/'.'config'.'/'.'db.config.php' );

//引入核心配置参数文件
require( $HYONEPATH.'/'.'config'.'/'.'c.config.php' );
...
function __autoload($class_name) {
	...
	if( !file_exists($file_path) ){
		throw new Exception('No such file as:'.$file_path);
	}
