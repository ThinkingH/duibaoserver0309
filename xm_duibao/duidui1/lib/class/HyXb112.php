		...
		if($usertype=='3'){//正常用户的注册
			...
			if($r===true){
				...
				if(count($userregisterlist)>0){
					...
					values ('".$this->phone."','".$userkey."','".parent::__get('create_datetime')."')";
			...
			if($r===true){
				...
				if($userphonelist['id']>0){//该手机号注册过，不更新临时用户的数据到正式用户表中
					...
					$userdatasql = "insert into xb_user (phone,tokenkey,create_datetime) values ('".$this->phone."','".$userkey."','".parent::__get('create_datetime')."')";
					$userdatalist = parent::__get('HyDb')->execute($userdatasql);
