...
class HyXb609 extends HyXb{
	...
	public function controller_getcollectionlist(){
		...
		if($this->collect=='1'){//用户进行收藏
			...
			$collectsql  = "select id from xb_collection where flag=2 and quanid='".$this->quanid."' and userid='".parent::__get('xb_userid')."' ";
			...
			if(count($collectlist)<=0){//该商品未被收藏过，点击收藏
				...
				$collectinsertsql = "insert into xb_collection (flag,userid,quanid,quantype,createtime) 
						values('2','".parent::__get('xb_userid')."','".$this->quanid."','".$this->quantype."','".$date."')";
			...
			if(count($collectlist)>0){//该商品存在，删除该商品
				...
				$delcollectsql  = "delete from xb_collection where flag=2 and quanid='".$this->quanid."' and userid='".parent::__get('xb_userid')."' ";
			...
			$historysql  = "delete from xb_collection where flag='2' and userid='".parent::__get('xb_userid')."' ";
