		...
		if($username!='' && $password!='') {
			...
			if($rootflag==9) {
				...
				if($lockflag==-1) {
					...
					session(HYSESSQZ.'checkflag',null); //账单审核权限判断字符串
		...
		if($this->_post('submit')!=null) {
			...
			if(!empty($list)) {
				if($passwd_md5==$list['passwd']) {
					...
					/* session('thesiteid',$list['thesiteid']); //渠道编号  */
					session('xingming',$list['xingming']); //姓名
					session('rootflag',$list['rootflag']); //权限标识
					session('lockflag',$list['lockflag']); //禁用标识
					session('resetflag',$list['resetflag']); //强制用户重置密码标识
					//将上次登录时间写入session中
					session('lastLoginTime',substr($list['lastLoginTime'],0,19));
					//将上次登录IP地址写入session中
					session('lastLoginIp',$list['lastLoginIp']);
					
					session(HYSESSQZ.'checkflag',$list['checkflag']); //账单审核权限判断字符串
					//如果是root用户，给予所有账单审核权限
					if($username=='root' && $list['rootflag']==9) {
						session(HYSESSQZ.'checkflag','YYYYYYYYYYYY'); //账单审核权限判断字符串
					}
