...
//手机发送验证码
	...
	$last_suc_time = isset($_SESSION['temp_success_time_20161010'])?$_SESSION['temp_success_time_20161010']:0;
	if(time()-$last_suc_time<=100) {
		echo 'error,发送太频繁，'.(time()-$last_suc_time).'秒后可再次发送';
		...
		if(count($list)>0){
			//说明该用户注册过
			//echo "<script>alert('该用户注册过，请直接登录！');window.location.href='login.php';</script>";
			echo 'resign';
			exit;
		}else{
			...
			if($httpcode == 200){
				...
				if($content!=''&& $content >0){
					...
					echo 'success';
					$_SESSION['temp_success_time_20161010'] = time();
				}else{
					echo 'fail';
				}
