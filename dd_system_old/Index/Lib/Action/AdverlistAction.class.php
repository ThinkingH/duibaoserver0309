...
class AdverlistAction extends Action {
	...
	public function index(){
		...
		foreach ($list as $keys=>$vals){
			...
			if(substr($list[$keys]['picurl'],0,7)!='http://'){
				$list[$keys]['picurl']=$arr['duibao-basic'].$list[$keys]['picurl'].'?imageView2/1/w/80/h/30/q/75|imageslim';
			}
	...
	public function adddata(){
		...
		$picurl=upload_qiniu('duibao-basic',$savepath,$savename);
	...
	//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
		...
		$delsql  = "select picurl from ad_advertisement where id='".$id."' ";
		$dellist = $Model->query($delsql);
		
		//云存储上删除
		delqiuniu('duibao-basic',$dellist[0]['picurl']);
