...
class PasswdrewriteAction extends Action {
	...
	public function index() {
		...
		$xingming = session(HYSESSQZ.'siteid');
		
		$Model = new Model();
		
		$datasql = "select * from shop_site where id='".$xingming."'";
		$datasql = $Model->query($datasql);
		
		$this -> assign('list',$datasql[0]);
	...
	public function rewritepasswd() {
		...
		if($this->_post('submit')!=null) {
			...
			//定义全局变量
			$username  = session(HYSESSQZ.'siteid');
			$password  = '';
			$repasswd  = '';
			$oldpasswd = '';
			...
			if($this->_post('passwd')!=null || $this->_post('repassword')!=null) {
				$passwd = $this -> _post('passwd');
				$repasswd = $this -> _post('repassword');
			}else {
			...
			//查询原密码是否输入正确
			$list  = $Model -> table('shop_site') -> where("id='".$username."'")->find();
			...
			if(!empty($list)) {
				if($oldpasswd_md5==$list['password']) {
					//修改密码
					
					//将新密码写入数组变量
					$pas['password']    = $passwd_md5;
					//$pas['qq']        = $qq;
					
					$ret   = $Model -> table('shop_site') -> where("id='".$username."'")->save($pas);
					...
					if($ret) {
						//重新设置session
						session(HYSESSQZ.'siteid',$username);
