...
//$url='http://'.$_SERVER['SERVER_NAME'].$_SERVER["REQUEST_URI"].':'.$_SERVER['REMOTE_PORT'];
...
if($err==''){
	...
	if(preg_match('/^('.str_replace(',','|',$upExt).')$/i',$extension))
	{
		...
		else
		{
			...
			if($msgType==1)$msg="'$targetPath'";/* $dir_url_http.$targetPath */
			else $msg="{'url':'".$dir_url_http.$targetPath."','localname':'".jsonString($localName)."','id':'1'}";//id参数固定不变，仅供演示，实际项目中可以是数据库ID
		}/* $msg="{'url':'".'/dd_system/Xheditor/demos/'.$targetPath."','localname':'".jsonString($localName)."','id':'1'}"; */
	}
