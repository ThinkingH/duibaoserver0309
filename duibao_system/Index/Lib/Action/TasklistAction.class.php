...
class TasklistAction extends Action{
	...
	public function updatetaskshow(){
		...
		Public function upload(){
			...
			//$upload->savePath =  './Public/Uploads/task/'.$id.'/';// 设置附件上传目录
			...
			//图片链接
			//$picurl = 'http://120.27.34.239:8009/dd_system';
			...
			for($i=0;$i<=5;$i++){
				...
				if($info[$i]['key']=='mainimage'){
					...
					$data['mainimage'] = str_replace('./','/',$pathname);
					...
					$pathname = $info[$i]['savepath'].$info[$i]['savename'];
					$data['liucheng_1_img'] = str_replace('./','/',$pathname);
					...
					$pathname = $info[$i]['savepath'].$info[$i]['savename'];
					$data['liucheng_2_img'] = str_replace('./','/',$pathname);
					...
					$pathname = $info[$i]['savepath'].$info[$i]['savename'];
					$data['liucheng_3_img'] = str_replace('./','/',$pathname);
					...
					$pathname = $info[$i]['savepath'].$info[$i]['savename'];
					$data['liucheng_4_img'] = str_replace('./','/',$pathname);
					...
					$pathname = $info[$i]['savepath'].$info[$i]['savename'];
					$data['liucheng_5_img'] = str_replace('./','/',$pathname);
				}
		...
		public function addtaskdata(){
			...
			$upload->maxSize  = 3145728 ;// 设置附件上传大小
			$upload->allowExts  = array('jpg', 'gif', 'png', 'jpeg','rar','pdf','txt');//设置附件上传类型
			//$upload->savePath =  './Public/Uploads/task//';// 设置附件上传目录
			$upload->savePath =  './Public/Uploads/task/'.date('Y-m').'/';// 设置附件上传目录
