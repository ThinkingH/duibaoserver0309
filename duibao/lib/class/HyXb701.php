...
class HyXb701 extends HyXb{
	...
	public function controller_getcollectlist(){
		...
		$collectsql  = "select flag,quanid,userid,quantype,createtime from xb_collection where flag=2 and userid = '".parent::__get('xb_userid')."' order by id desc limit $firstpage,$pagesize ";
		...
		//查询优惠券表
		$youhuiquandatasql  = "select id,title,picurl,yuanprice,nowprice,yilingcon,type from z_quanmainlist where flag='1' and zflag='1' and z='".$z."' and id in $quanidarr ";
		...
		foreach ($youhuiquandatalist as $keys => $vals){
			...
			$youhuiquandatalist[$keys]['nowprice'] = $youhuiquandatalist[$keys]['yuanprice']-$youhuiquandatalist[$keys]['nowprice'];
		}
