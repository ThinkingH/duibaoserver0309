...
$url = 'http://www.meitan315.com/web/coalStoreAction_index.html';
...
$contenttt = explode('<div class="tabBox" style="display: block;">',$content);

$lastcontent = explode('<div class="tabBox">',$contenttt[1]);



$pat_t = '/<td class="td-2">(.{1,30})<\/td>/';
preg_match_all($pat_t,$lastcontent[0],$iparr_t1);

$pat_t = '/<td class="td-3">(.{1,50})<\/td>/';
preg_match_all($pat_t,$lastcontent[0],$iparr_t2);

$pat_t = '/<td class="td-4">(.{1,50})<\/td>/';
preg_match_all($pat_t,$lastcontent[0],$iparr_t3);

$pat_t = '/<td class="td-5">(.{1,50})<\/td>/';
preg_match_all($pat_t,$lastcontent[0],$iparr_t4);
...
foreach($iparr_t1[1] as $keyt => $valt) {
	...
	$t2 = isset($iparr_t2[1][$keyt])?$iparr_t2[1][$keyt]:'';
	
	$t3 = isset($iparr_t3[1][2*$keyt])?$iparr_t3[1][2*$keyt]:'';//发热量
	$t4 = isset($iparr_t3[1][2*$keyt+1])?$iparr_t3[1][2*$keyt+1]:'';//本期价格
	
	$t6 = isset($iparr_t4[1][2*$keyt])?$iparr_t4[1][2*$keyt]:'';//上气价格
	$t7 = isset($iparr_t4[1][2*$keyt+1])?$iparr_t4[1][2*$keyt+1]:'';//去年同期
	
	
	
	
	$md5 = md5($t1.$t2.$t3.$t4.$t6.$t7);
	
	
	$sql_insert = "insert into zt_xianhuo (md5id,gangkou,type,fareliang,nowprice,lastprice,oldprice) values(
			'".$md5."','".$t1."','".$t2."','".$t3."','".$t4."','".$t6."','".$t7."'
			)";
	//echo $sql_insert.'<br>';
