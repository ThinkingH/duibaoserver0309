<?php

// 页面超时设置
set_time_limit(600);

//设置时区
date_default_timezone_set('Asia/Shanghai');



require_once('./HyItems.php');
require_once('./HyDb.php');


$tmppathurl = './tmp/zhuaqu1_temp';

//存放对应的数据链接
$urlarr = array();

//---------------------------------------------------------------------------------------
//秦皇岛煤炭网今日关注
$liebiaourl = 'http://news.cqcoal.com/a/xinwenzixun/jinriguanzhu/';
$tempurlarr = func_zixunurlchuli($liebiaourl);

foreach($tempurlarr as $valt) {
	array_push($urlarr,$valt);
}

//---------------------------------------------------------------------------------------
//秦皇岛煤炭网煤炭经济
$liebiaourl = 'http://news.cqcoal.com/a/xinwenzixun/shichangguancha/';
$tempurlarr = func_zixunurlchuli($liebiaourl);

foreach($tempurlarr as $valt) {
	array_push($urlarr,$valt);
}

//---------------------------------------------------------------------------------------
//秦皇岛煤炭网煤炭供需
$liebiaourl = 'http://news.cqcoal.com/a/xinwenzixun/meitanzixun/';
$tempurlarr = func_zixunurlchuli($liebiaourl);

foreach($tempurlarr as $valt) {
	array_push($urlarr,$valt);
}

//---------------------------------------------------------------------------------------
//秦皇岛煤炭网煤炭运输
$liebiaourl = 'http://news.cqcoal.com/a/xinwenzixun/meitanyunshu/';
$tempurlarr = func_zixunurlchuli($liebiaourl);

foreach($tempurlarr as $valt) {
	array_push($urlarr,$valt);
}

//---------------------------------------------------------------------------------------
//秦皇岛煤炭网国际煤炭
$liebiaourl = 'http://news.cqcoal.com/a/xinwenzixun/guojimeitan/';
$tempurlarr = func_zixunurlchuli($liebiaourl);

foreach($tempurlarr as $valt) {
	array_push($urlarr,$valt);
}


if(count($urlarr)>0) {
	//数据库初始化
	$HyDb = new HyDb();
	
}




foreach($urlarr as $valu) {
	
	$valu_url        = $valu['url'];
	$valu_title      = $valu['title'];
	$valu_createtime = $valu['createtime'];
	
	//查询数据库看该链接是否已经存在
	$sql_pan = "select id from zt_wx_news where y_url='".$valu_url."'";
	$list_pan = $HyDb->get_one($sql_pan);
	
	if($list_pan<=0) {
		//执行数据抓取
		$res = HyItems::vget($valu_url);
		$content = HyItems::hy_tonull($res['content']);
		//时间
		$temp_neirong_arr1 = explode('<p>',$content);
		$temp_neirong_arr2 = explode('</div>',$temp_neirong_arr1[1]);;
		$temp_neirong = rtrim($temp_neirong_arr2[0],'</p>');
		
		$biaoti = $valu_title;
		$riqi = $valu_createtime;
		$neirong = preg_replace("/<a[^>]*>(.*)<\/a>/isU",'${1}',$temp_neirong); //去除a标签
		$neirong = iconv('GBK', 'UTF-8', $neirong); //编码转换
		if(strlen($neirong)>50) {
			$neirong .= '  （秦皇岛煤炭网）';
		}else {
			$neirong = '';
		}
		if(strlen($biaoti)<9) {
			$biaoti = '';
		}
		if($biaoti!='' && $riqi!='' && $neirong!='') {
		
			$jiaoyanstring = $biaoti.$riqi.$neirong;
			$md5crc32 = md5($jiaoyanstring).crc32($jiaoyanstring);
			//$intorder = strtotime($riqi).mt_rand(1000,9999);
			$intorder = time().mt_rand(1000,9999);
		
			$sql_pan = "select id from zt_wx_news where md5crc32='".$md5crc32."' and typeid='2' ";
			$list_pan = $HyDb->get_one($sql_pan);
		
			if($list_pan<=0) {
					
				//typeid 1为纯文本推送，2为链接
				$sql_insert = "insert into zt_wx_news (typeid,intorder,contenttime,title,content,
							y_url,create_datetime,md5crc32) values
							('2','".$intorder."','".$riqi."','".$biaoti."','".$neirong."',
							'".$valu_url."','".date('Y-m-d H:i:s')."','".$md5crc32."')";
				$HyDb->execute($sql_insert);
				//echo $sql_insert;
				echo '-';
					
			}else {
				echo '|';
			}
		
		}else {
			echo '#';
		}
		
		
// 		echo '<textarea>';
// 		print_r($content);
// 		echo '</textarea>';
		
	}else {
		echo 'J';
	}
	
	
	ob_flush();
	flush();
	
	sleep(1);
	
}




function func_zixunurlchuli($liebiaourl='') {
	if($liebiaourl=='') {
		return array();
	}
	
	
	$urlarr = array();
	
	$res = HyItems::vget($liebiaourl);
	$content = $res['content'];
	
	$temp1arr = explode('<li><a href="',$content);
	
	foreach($temp1arr as $valt) {
		if(substr($valt,0,3)=='/a/') {
			//链接
			$temp_url_arr1 = explode('"',$valt);
			$temp_url = $temp_url_arr1[0];
			//标题
			$temp_title_arr1 = explode('class="title">',$valt);
			$temp_title_arr2 = explode('</a><span>',$temp_title_arr1[1]);;
			$temp_title = $temp_title_arr2[0];
			$temp_title = iconv('GBK', 'UTF-8', $temp_title); //编码转换
			//时间
			$temp_time_arr1 = explode('</a><span>',$valt);
			$temp_time_arr2 = explode('</span></li>',$temp_time_arr1[1]);;
			$temp_time = $temp_time_arr2[0];
	
	
			if(strtotime($temp_time)>=(time()-6*60*60)) {
				//八小时之前到现在的数据
				$temparr = array(
						'url' => 'http://news.cqcoal.com'.$temp_url,
						'title' => trim($temp_title),
						'createtime' => trim($temp_time),
				);
				array_push($urlarr,$temparr);
					
					
			}else {
				//跳过，不作处理
				
			}
	
	
		}
	
	}
	
	
	return $urlarr;
	
	
}











exit;

$urlarr = array();

for($i=1;$i<=2;$i++) {
	
	$postdata = 'pageVO.toPage='.$i;
	
	
	$res = HyItems::vpost($zixunurl,$postdata);
	$content = $res['content'];
	
	$temp1arr = explode('<div class="f_hqzx_title clearfix"><a href="',$content);
	
	foreach($temp1arr as $valt) {
		if(substr($valt,0,33)=='/new/infoQuotesAction_detail.html') {
			$temp2arr = explode('"',$valt);
			array_push($urlarr,$temp2arr[0]);
		}
	}
	
	
}


if(count($urlarr)>0) {
	//数据库初始化
	$HyDb = new HyDb();
}



foreach($urlarr as $valu) {
	
	$mesurl = 'http://www.315dz.com/'.$valu;
	//echo $mesurl.'<br>';
	$res = HyItems::vget($mesurl);
	$content = $res['content'];
	
	$temp1arr = explode('<iframe width="100%" src="',$content);
	$temp2arr = explode('"',$temp1arr[1]);
	
	$y_url = 'http://www.315dz.com/'.$temp2arr[0];
	
	$res = HyItems::vget($y_url);
	$content = HyItems::hy_tonull($res['content']);
	
	
	$biaoti_temp = explode('<h2 class="tit_01" id="cont_title">',$content);
	$biaoti_temp1 = explode('</h2>',$biaoti_temp[1]);
	$biaoti = trim($biaoti_temp1[0]);
	
	$riqi_temp = explode('<div class="time_lyfl c666" id="cont_riqi">',$content);
	$riqi_temp1 = explode('来源：',$riqi_temp[1]);
	$riqi = trim($riqi_temp1[0]);
	
	$neirong_temp = explode('<div id="frameContent" class="txt_0422">',$content);
	$neirong_temp1 = explode('      </div>',$neirong_temp[1]);
	$neirong = trim($neirong_temp1[0]);
	
	
// 	echo $biaoti.'<br>';
// 	echo $riqi.'<br>';
// 	echo $neirong.'<br>';
	
	if($biaoti!='' && $riqi!='' && $neirong!='') {
		
		$jiaoyanstring = $biaoti.$riqi.$neirong;
		$md5crc32 = md5($jiaoyanstring).crc32($jiaoyanstring);
		$intorder = strtotime($riqi).mt_rand(1000,9999);
		
		$sql_pan = "select id from zt_wx_news where md5crc32='".$md5crc32."' and typeid='2' ";
		$list_pan = $HyDb->get_one($sql_pan);
		
		if($list_pan<=0) {
			
			//typeid 1为纯文本推送，2为链接
			$sql_insert = "insert into zt_wx_news (typeid,intorder,contenttime,title,content,
							y_url,create_datetime,md5crc32) values 
							('2','".$intorder."','".$riqi."','".$biaoti."','".$neirong."',
							'".$y_url."','".date('Y-m-d H:i:s')."','".$md5crc32."')";
			$HyDb->execute($sql_insert);
			echo '-';
			
		}else {
			echo '|';
		}
	
	}else {
		echo '#';
	}
	
	ob_flush();
	flush();
	
// 	echo '<textarea>';
// 	print_r($content);
// 	echo '</textarea>';
	
// 	exit;
}





echo 'ok';






// echo '<textarea>';
// print_r($urlarr);
// echo '</textarea>';










