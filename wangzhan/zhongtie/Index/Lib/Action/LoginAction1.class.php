...
class LoginAction extends Action {
	...
	public function login(){
		...
		if($submit!=''){
			...
			//判断该账户是否存在
			$sqlaccount  = "select phone from users where phone='".$account."' and password='".$password."' ";
			print_r($sqlaccount);
			/* exit;
	...
	public function register(){
		...
		//注册的信息存入数据库
		$sqlphonedata = "insert into users (phone,password,createtime) values ('".$phone."','".$password."','".$date."')";
	...
	public function message(){
		...
		$zuoji1    = $this->_post('zuoji1');
		$zuoji      = $this->_post('zuoji');
		$qq         = $this->_post('QQ');
		...
		if($submit!=''){
			...
			if($zuoji1==''|| $zuoji==''){
				echo "<script>alert('请填写完整座机电话！'); history.go(-1);</script>";
				$this->error('请填写完整座机电话！');
			...
			//数据插入到数据表中
			$sqluser = "select id from users order by id desc";
			$listuser = $Model->query($sqluser);
			if($listuser[0]['id']>0){
			//echo $listuser[0]['id'];
				$sqlmess= "update users set company='".$company."',lianxiren='".$name."',
						zuojiphone='".$tel."',email='".$email."',qq='".$qq."',address='".$adcompany."'
	...
	public function order(){
		...
		foreach ($listorder as $key=>$val){
			...
			if($listorder[$key]['flag']=='10'){
				$listorder[$key]['flags']='发布成功，等待初审！';
			}
			
			if($listorder[$key]['flag']=='20'){
				$listorder[$key]['flags']='通过初审，等待复审！';
			}
			
			if($listorder[$key]['flag']=='30'){
				$listorder[$key]['flags']='通过复审，等待终审!';
			}
			
			if($listorder[$key]['flag']=='40'){
					$listorder[$key]['flags']='订单完成!';
			}
	...
	Public function upload(){
		...
		$data=array();
		$path="http://127.0.0.1:8001/zhongtie";
	      
		for($i=0;$i<=4;$i++){
			
			if($info[$i]['key']=='file1'){//营业执照
				$lujing1=$path.$info[$i]['savepath'].$info[$i]['savename'];
				$data['businesslicence'] = str_replace('./','/',$lujing1);
			}else if($info[$i]['key']=='file2'){//库存证明
				$lujing2=$path.$info[$i]['savepath'].$info[$i]['savename'];
				$data['danzi'] =str_replace('./','/',$lujing2);
			}else if($info[$i]['key']=='file3'){//购货发票
				$lujing3=$path.$info[$i]['savepath'].$info[$i]['savename'];
				$data['buybill'] = str_replace('./','/',$lujing3);
			}else if($info[$i]['key']=='file4'){//煤炭产地
				$lujing4=$path.$info[$i]['savepath'].$info[$i]['savename'];
				$data['coalzhengming'] = str_replace('./','/',$lujing4);
			}else if($info[$i]['key']=='file5'){//质检报告
				$lujing5=$path.$info[$i]['savepath'].$info[$i]['savename'];
