...
$sql_get = "select zid from xinwenzixun where flag=9 limit 30";
...
foreach($list_zid as $val) {
	...
	$url = 'http://www.zhaomei.com/news/view/'.$zid;
	...
	$pat_t = '/<div class="ui-contentBd mt5">(.*)<\/div>                    <\/div>                <\/div>                <div class="bottom-share">/';
	preg_match_all($pat_t,$content,$iparr_t1);
	
	$newstr = preg_replace("/<img.+?\/>/", "", $iparr_t1[1][0]);
	$newstr = preg_replace("/<a[^>]*href=[^>]*>|<\/[^a]*a[^>]*>/i","",$newstr);
	
	$pat_t = '/<span class="ui-titTag m-linkA"><a href="#">(.{14,20})<\/a><\/span>/';
	preg_match_all($pat_t,$content,$iparr_t2);
	
	$pat_t = '/<span class="ui-titFrom m-linkB"><a>(.{1,45})<\/a><\/span>/';
	preg_match_all($pat_t,$content,$iparr_t3);
	
	
	$content = trim($newstr);
	$writeuser = isset($iparr_t3[1][0])?$iparr_t3[1][0]:'';
	$sendtime  = isset($iparr_t2[1][0])?$iparr_t2[1][0]:'';
	
	
	if(strlen($content)>88) {
		$sql_update = "update xinwenzixun set content='".$content."',writeuser='".$writeuser."',sendtime='".$sendtime."',flag=1 
						where zid='".$zid."'";
		//echo $sql_update;
	...
	}else if(strlen($content)<=88 && strlen($content)>5) {
		$sql_update = "update xinwenzixun set content='".$content."',writeuser='".$writeuser."',sendtime='".$sendtime."',flag=4 
						where zid='".$zid."'";
		//echo $sql_update;
		$HyDb->execute($sql_update);
