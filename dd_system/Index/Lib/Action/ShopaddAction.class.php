...
class ShopaddAction extends Action{
	...
	public function index(){
		...
		//商品虚实
		$xushi_type_arr = array(
				'1' => '虚拟',
				'2' => '实物',
		);
		$optionxushi = '<option value=""></option>';
		foreach($xushi_type_arr as $keyc => $valc) {
			$optionxushi .= '<option value="'.$keyc.'" ';
			if($xushi==$keyc) { $optionxushi .= ' selected="selected" '; }
			$optionxushi .= '>'.$valc.'</option>';
		}
		...
		//渠道编号
		$sitesql = "select id,lianxiren,flag from shop_site where flag='1' and checkstatus='2' and storestatus='2' ";
		...
		foreach($sitelist as $keyc => $valc) {
			$optionsiteid .= '<option value="'.$valc['id'].'" ';
			 if($siteid==$valc['id']) { $optionsiteid .= ' selected="selected" '; } 
		...
		if($xushi!=''){
			$sql_where .= "xushi_type='".$xushi."' and ";
		}
		
		if($productid!=''){
			$sql_where .= "id='".$productid."' and ";
		}
		...
		foreach ($sitelist as $keys=>$vals){
			$sitearr[$sitelist[$keys]['id']] = $sitelist[$keys]['id'].'-'.$sitelist[$keys]['lianxiren'];
			...
			for ($i=0;$i<=3;$i++){
				...
				if($info[$i]['key']=='mainpic'){
					...
					if(substr($dellist[0]['mainpic'],0,7)!='http://'){
						//七牛云存储上图片的删除
						delete_qiniu('duibao-shop',$dellist[0]['mainpic']);
					}
				...
				if($info[$i]['key']=='showpic1'){
					...
					if(substr($dellist[0]['showpic1'],0,7)!='http://'){
						...
						//七牛云存储上图片的删除
						delete_qiniu('duibao-shop',$dellist[0]['showpic1']);
					}
				...
				if($info[$i]['key']=='showpic2'){
					...
					if(substr($dellist[0]['showpic2'],0,7)!='http://'){
						...
						//七牛云存储上图片的删除
						delete_qiniu('duibao-shop',$dellist[0]['showpic2']);
					}
				...
				if($info[$i]['key']=='showpic3'){
					...
					if(substr($dellist[0]['showpic3'],0,7)!='http://'){
						...
						//七牛云存储上图片的删除
						delete_qiniu('duibao-shop',$dellist[0]['showpic3']);
					}
