...
class QuantypeAction extends Action {
	...
	public function index(){
		...
		foreach ($sellist as $keys => $vals){
			...
			$sellist[$keys]['smallpic'] = hy_qiniuimgurl('duibao-basic',$sellist[$keys]['smallpic'],'100','80');
	...
	public function updatetypeshow(){
		...
		if(count($quantypelist)<=0){
			...
			$quantypelist[0]['smallpic'] = hy_qiniuimgurl('duibao-basic',$quantypelist[0]['smallpic'],'80','60');
		}
	...
	public function deletetypedata(){
		...
		if($delete_submit!=''){
			...
			$ret_zhuangtai = M("xb_kind")->where("kindtype='".$kindtype."'")->find();
