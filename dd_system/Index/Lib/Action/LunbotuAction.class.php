...
class LunbotuAction extends Action {
	...
	public function index(){
		...
		foreach ($list as $keys=>$vals){
			...
			//600 300
			$list[$keys]['img'] = hy_qiniuimgurl('duibao-basic',$list[$keys]['img'],'180','60');
		...
		foreach ($hot_list as $keys=>$vals){
			...
			$hot_list[$keys]['img'] = hy_qiniuimgurl('duibao-basic',$hot_list[$keys]['img'],'100','80');
		}
		//特供优惠
		foreach ($youhui_list as $keys=>$vals){
			...
			$youhui_list[$keys]['img'] = hy_qiniuimgurl('duibao-basic',$youhui_list[$keys]['img'],'190','80');
		...
		foreach ($chouprize_list as $keys=>$vals){
			...
			$chouprize_list[$keys]['img'] = hy_qiniuimgurl('duibao-basic',$chouprize_list[$keys]['img'],'300','100');
		}
	...
	public function updatelunbotushow(){
		...
		if(count($verdata_list)<=0){
			...
			if($verdata_list[0]['biaoshi']=='5'){
				$verdata_list[0]['img'] = hy_qiniuimgurl('duibao-basic',$verdata_list[0]['img'],'100','80');
			}else{
				$verdata_list[0]['img'] = hy_qiniuimgurl('duibao-basic',$verdata_list[0]['img'],'150','60');
			}
